...
- 新增 8 项补图脚本级回归：临时项目执行真实入口，以模拟 fetch 和假 Key 测试 401/403、全批断网、损坏图片、TMDB ID 不匹配、并发写入保护、允许的 404 及成功写入。验证失败时原文件字节不变（并发情形保留外部写入），无残留临时文件、日志不输出假 Key；不使用真实代理或仓库快照。另新增 9 项 Trakt 真实入口回归：模拟 Playwright 模块与时钟，覆盖缺页、无终页、重复 ID、记录缩减、空记录、导航失败、HTTP 503、损坏 JSON，以及早期声明 51 页但实际第 2 页为终页的成功路径。失败时原快照字节不变、浏览器关闭、无临时目录残留；成功路径验证真实写入。模拟浏览器不代表真实 Chrome 网络行为验收。再新增 7 项补图回归，总计 30 项测试：429/503 验证最多 3 次请求及 1000/2000ms 退避参数（模拟计时器，不真实等待）；恢复成功与耗尽保留原文各一项。混合 404/网络失败批次允许写入已验证海报，失败条目保留原值并列入报告；混合认证失败即使另有成功项也中止整个写入。这是现有分层失败策略，不代表所有网络失败都会中止。仍未覆盖真实 Chrome 网络故障、所有 HTTP 状态组合与文件系统故障注入。
