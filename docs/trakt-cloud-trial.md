...
先检查 PR 的 Verify Astro site。若显示 awaiting approval，由维护者审阅并批准运行；若没有检查，可由维护者关闭再重开 PR。不要关闭仓库安全保护。当前流程仍可能需要人工批准，尚不是无人值守。

候选 JSON 在创建 PR 前复制到 runner 临时目录，从该副本上传，避免 PR Action 恢复基准 checkout 后上传旧数据。Artifact 保留 7 天。PR 仅提交观影 JSON。

## 尚待验证及完善

- 按需模式的真实云端运行、重复运行无提交、Artifact 与 PR 内容一致性。
